...
public class RenderConstruct implements ISimpleBlockRenderingHandler
{
	@Override
	...
	{
		TileEntityConstruct te = (TileEntityConstruct) renderer.blockAccess.getTileEntity(i, j, k);
		...
		for(int index = 0; index < dd;)
		{
			...
			if(te.solidCheck[in3])
			{
				...
				maxZ = minZ + div;
				if(!breaking)
					renderer.overrideBlockTexture = HomeImprovement.WoodConstruct.getIcon(0, te.materialTypes[index]);
				index++;
				render = true;
			}
		...
		for(int index = 0; index < dd; )
		{
			if(te.solidCheck[(dd+index) >> 3])
			{
				...
				maxZ = minZ + div;
				if(!breaking)
					renderer.overrideBlockTexture = HomeImprovement.WoodConstruct.getIcon(0, te.materialTypes[index + dd]);
				index+=8;
				render = true;
			}
		...
		for(int index = 0; index < dd;)
		{
			if(te.solidCheck[(dd2+index) >> 3])
			{
				...
				maxZ = 1;
				if(!breaking)
					renderer.overrideBlockTexture = HomeImprovement.WoodConstruct.getIcon(0, te.materialTypes[index + dd2]);
				index+=8;
				render = true;
			}
