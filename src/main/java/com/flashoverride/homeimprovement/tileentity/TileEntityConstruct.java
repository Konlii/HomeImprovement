...
public class TileEntityConstruct extends NetworkTileEntity
{
	...
	/*private void updateClient()
	{
		...
		for(int layer = 0; layer < 24; layer++)
		{
			...
			for(int index = 0; index < 8 && solidCheck[layer] == true; index++)
			{
				...
				if(!data.get(in) || (type != -1 && materialTypes[in] != type))
					solidCheck[layer] = false;
				type = materialTypes[in];
			}
		}
		for(int layer = 0; layer < 24; layer++)
		{
	...
	public void ejectContents()
	{
		for(int i = 0; i < 192; i++)
		{
			if(data.get(i))
			{
				...
				data.clear(i);
				stack = new ItemStack(HomeImprovement.SinglePlank, 1, materialTypes[i]);
	...
	{
		inStream.readFully(materialTypes, 0, 192);
